...
(function() {
    ...
    Render.create = function(options) {
        ...
        encoder = new GIFEncoder(render.options.width, render.options.height);

        // stream the results as they are available into gif
        encoder.createReadStream().pipe(fs.createWriteStream(render.options.outputFile));
        encoder.start();
        encoder.setRepeat(render.options.repeat);   // 0 for repeat, -1 for no-repeat
        encoder.setDelay(render.options.frameDelay);  // frame delay in ms
     ...
     * @param {render} render
     */
    Render.world = async function(render, time) {
